...
const getAllProperties = async(req, res) => {
    ...
    if(title_like) {
        //$options: 'i' znači da ne gleda na uppercase i lowercase prilikom searcha
...
const createProperty = async(req, res) => {
    ...
    try {
        ...
        //Start a new session...
        //Session pomaže da proces radi il ne radi... da ne zapadne negdje između
...
const deleteProperty = async(req, res) => {
    try {
        ...
        await Property.deleteOne({_id: id}, {session}); // using remove method on Property model with query object
        propertyToDelete.creator.allProperties.pull(propertyToDelete);

        await propertyToDelete.creator.save({session});
        await session.commitTransaction();
        res.status(200).json({ message: 'Property deleted sucesfully'});
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
};
